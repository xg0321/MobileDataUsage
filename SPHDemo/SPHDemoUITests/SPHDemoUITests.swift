...
class SPHDemoUITests: XCTestCase {
    ...
    func testExample() {
        ...
        var tabelExist = app.tables.element(boundBy: 0).waitForExistence(timeout: 30)
        var retry = 2
        while !tabelExist && retry > 0 {
            app.buttons["刷新"].tap()
            tabelExist = app.tables.element(boundBy: 0).waitForExistence(timeout: 30)
            retry -= 1
        }
        if tabelExist {
            tabelExistHandler()
